...
public class QuestionService {
	...
	sorts.add(Sort.Order.desc("notice"));
	...
	/**
	 * 게시글의 공지 상태를 한 번에 갱신한다.
	 */
	public void updateNoticeStatus(Collection<Integer> questionIds, boolean notice) {
		if (questionIds == null || questionIds.isEmpty()) {
			return;
		}
		List<Question> targets = this.questionRepository.findAllById(questionIds);
		for (Question question : targets) {
			question.setNotice(notice);
		}
		this.questionRepository.saveAll(targets);
	}

	/**
	 * 게시글을 ID 목록 기준으로 일괄 삭제한다.
	 */
	public void deleteAllByIds(Collection<Integer> questionIds) {
		if (questionIds == null || questionIds.isEmpty()) {
			return;
		}
		this.questionRepository.deleteAllById(questionIds);
	}

	/**
	 * 관리자 화면에서 사용할 공지 우선 정렬 리스트를 반환한다.
	 */
	public Page<Question> getQuestionsForAdmin(int page, int size) {
		int safePage = Math.max(page, 0);
		int safeSize = size < 1 ? 50 : size;
		// 공지글을 우선 노출하고 생성일 역순으로 정렬해 최신 공지를 빠르게 확인할 수 있도록 구성한다.
		Sort sort = Sort.by(Sort.Order.desc("notice"), Sort.Order.desc("createDate"));
		Pageable pageable = PageRequest.of(safePage, safeSize, sort);
		return this.questionRepository.findAll(pageable);
	}

	/**
	 * 관리자 화면에서 사용할 공지 우선 정렬 전체 리스트를 반환한다.
	 * - 신규 페이징 로직과의 호환을 위해 기존 호출부가 있을 경우를 대비해 유지한다.
	 */
	public List<Question> getQuestionsForAdmin() {
		Sort sort = Sort.by(Sort.Order.desc("notice"), Sort.Order.desc("createDate"));
